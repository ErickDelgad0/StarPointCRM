...
// Get the contact from the Employee table
...
if (isset($_POST['submit'])) {
    ...
    if (!$error_msg) {
        ...
        $stmt->execute();
        // Get the updated contact from the Employee table
        $stmt = $pdo->prepare('SELECT * FROM  ' . $Employee . ' WHERE id = ?');
        $stmt->execute([ $_GET['id'] ]);
        $contact = $stmt->fetch(PDO::FETCH_ASSOC);
        // Output message
    ...
    <div id="content">
        ...
        <div class=container-fluid>
            ...
            <div class="d-sm-flex align-items-center justify-content-between mb-4">
                <div class="wrap">
                    <h1 class="h3 mb-0 text-gray-800">Update Employee Contact #<?=$contact['id']?></h1>
                </div>
            </div>
            ...
            <form action="?id=<?=$contact['id']?>" method="post" class="crud-form">
                ...
                <a href="employee-password-reset.php?id=<?=$contact['id']?>" class="btn">Reset Password</a>
