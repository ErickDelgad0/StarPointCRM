...
if (isset($_GET['bulk_action'])) {
	...
	if ($ids) {
		...
		if ($_GET['bulk_action'] == 'delete') {
			...
			$stmt = $pdo->prepare('DELETE FROM ' . $Leads . ' WHERE id IN (' . implode(',', array_fill(0, count($ids), '?')) . ')');
			$stmt->execute($ids);
		}
		...
		if ($_GET['bulk_action'] == 'export') {
			...
			$stmt = $pdo->prepare('SELECT * FROM ' . $Leads . ' WHERE id IN (' . implode(',', array_fill(0, count($ids), '?')) . ')');
			$stmt->execute($ids);
			...
			fputcsv($output, array_keys($Leads_Columns));
		...
		if ($_GET['bulk_action'] == 'edit') {
			...
			header('Location: leads-bulk-update.php?ids=' . implode(',', $ids));
			exit;
		}
	}
}
...
$order_by = isset($_GET['order_by']) && in_array($_GET['order_by'], array_keys($Leads_Columns)) ? $_GET['order_by'] : $default_leads_column;
...
$stmt = $pdo->prepare('SELECT * FROM ' . $Leads . $where_sql . ' ORDER BY ' . $order_by . ' ' . $order_sort . $limit_sql);
...
$stmt = $pdo->prepare('SELECT COUNT(*) FROM ' . $Leads . $where_sql);
// Bind the search query param to the SQL query
if (isset($_GET['search']) && !empty($_GET['search'])) {	
	$stmt->bindValue(':search_query', '%' . $_GET['search'] . '%');
}
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <form action="" method="get" class="crud-form">
                        ...
                        <div class="top">
                            <div class="btns">
                                <a href="leads-create.php" class="btn">Create Contact</a>
                            </div>
                            <div class="wrap">
                                <div class="filters">
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <td<?=$order_by==$column_key?' class="active"':''?>>
                                            ...
                                            <a href="leads.php?page=1&records_per_page=<?=$records_per_page?>&order_by=<?=$column_key?>&order_sort=<?=$order_sort == 'ASC' ? 'DESC' : 'ASC'?>&from_date=<?=$from_date?>&to_date=<?=$to_date?><?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>">
                                            <?php endif; ?>
                                                <?=$column['label']?>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="actions">
                                            ...
                                            <a href="leads-delete.php?id=<?=$result['id']?>" class="trash"><i class="fa-solid fa-xmark fa-xs"></i></a>
                                        </td>
                                    </tr>
                                    <?php endforeach; ?>
                                </tbody>
                        ...
                        <div class="bot">
                            <div class="records-per-page">
                                ...
                                <a href="leads.php?page=1&records_per_page=5&order_by=<?=$order_by?>&order_sort=<?=$order_sort?>&from_date=<?=$from_date?>&to_date=<?=$to_date?><?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>">5</a>
                                <a href="leads.php?page=1&records_per_page=10&order_by=<?=$order_by?>&order_sort=<?=$order_sort?>&from_date=<?=$from_date?>&to_date=<?=$to_date?><?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>">10</a>
                                <a href="leads.php?page=1&records_per_page=20&order_by=<?=$order_by?>&order_sort=<?=$order_sort?>&from_date=<?=$from_date?>&to_date=<?=$to_date?><?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>">20</a>
                                <a href="leads.php?page=1&records_per_page=50&order_by=<?=$order_by?>&order_sort=<?=$order_sort?>&from_date=<?=$from_date?>&to_date=<?=$to_date?><?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>">50</a>
                                <a href="leads.php?page=1&records_per_page=100&order_by=<?=$order_by?>&order_sort=<?=$order_sort?>&from_date=<?=$from_date?>&to_date=<?=$to_date?><?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>">100</a>
                                <a href="leads.php?page=1&records_per_page=all&order_by=<?=$order_by?>&order_sort=<?=$order_sort?>&from_date=<?=$from_date?>&to_date=<?=$to_date?><?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>">all</a>
                            </div>
                            <div class="pagination">
                                <?php if ($records_per_page != 'all'): ?>
                                <?php if ($page > 1): ?>
                                <a href="leads.php?page=<?=$page-1?>&records_per_page=<?=$records_per_page?>&order_by=<?=$order_by?>&order_sort=<?=$order_sort?>&from_date=<?=$from_date?>&to_date=<?=$to_date?><?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>" class="prev">
                                    <i class="fa-solid fa-angle-left"></i> Prev
                                </a>
                                <?php endif; ?>
                                <?php if ($page > 1): ?>
                                <a href="leads.php?page=1&records_per_page=<?=$records_per_page?>&order_by=<?=$order_by?>&order_sort=<?=$order_sort?>&from_date=<?=$from_date?>&to_date=<?=$to_date?><?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>">1</a>
                                <?php endif; ?>
                                <?php if ($page > 2): ?>
                                <div class="dots">...</div>
                                <?php if ($page == ceil($num_results/$records_per_page) && ceil($num_results/$records_per_page) > 3): ?>
                                <a href="leads.php?page=<?=$page-2?>&records_per_page=<?=$records_per_page?>&order_by=<?=$order_by?>&order_sort=<?=$order_sort?>&from_date=<?=$from_date?>&to_date=<?=$to_date?><?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>"><?=$page-2?></a>
                                <?php endif; ?>
                                <?php endif; ?>
                                <?php if ($page-1 > 1): ?>
                                <a href="leads.php?page=<?=$page-1?>&records_per_page=<?=$records_per_page?>&order_by=<?=$order_by?>&order_sort=<?=$order_sort?>&from_date=<?=$from_date?>&to_date=<?=$to_date?><?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>"><?=$page-1?></a>
                                <?php endif; ?>
                                <a href="leads.php?page=<?=$page?>&records_per_page=<?=$records_per_page?>&order_by=<?=$order_by?>&order_sort=<?=$order_sort?>&from_date=<?=$from_date?>&to_date=<?=$to_date?><?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>" class="selected"><?=$page?></a>
                                ...
                                <a href="leads.php?page=<?=ceil($num_results/$records_per_page)?>&records_per_page=<?=$records_per_page?>&order_by=<?=$order_by?>&order_sort=<?=$order_sort?>&from_date=<?=$from_date?>&to_date=<?=$to_date?><?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>"><?=ceil($num_results/$records_per_page)?></a>
                                <?php endif; ?>
                                <?php if ($records_per_page != 'all' && $page < ceil($num_results/$records_per_page)): ?>
                                <a href="leads.php?page=<?=$page+1?>&records_per_page=<?=$records_per_page?>&order_by=<?=$order_by?>&order_sort=<?=$order_sort?>&from_date=<?=$from_date?>&to_date=<?=$to_date?><?=isset($_GET['search']) ? '&search=' . htmlentities($_GET['search'], ENT_QUOTES) : ''?>" class="next">
